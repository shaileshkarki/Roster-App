...
function Roster({
    ...
    const transformTimes = () => {
        ...
        for (let i = 0; i < shifts.length; i++) {
            ...
            transformedData[i].end_time = moment.unix(
                transformedData[i].end_time
            );
    ...
    const handleSubmit = async (e) => {
        ...
        if (create) {
            try {
                const response = await axios.post(`${HOST_URL}roster`, {
                    shifts: items,
                    weekNumber,
                    title,
                    startDate,
            ...
            try {
                const response = await axios.put(`${HOST_URL}roster`, {
                    shifts: items,
                    weekNumber,
                });
                if (response.status === 200) {
                        ...
                        <div
                            {...getItemProps({
                                style: {
                                    ...
                                    // borderWidth: 1,
                                    borderRadius: 4,
                                    borderLeftWidth: itemContext.selected
                                        ? 3
                                        : 1,
                                    borderRightWidth: itemContext.selected
                                        ? 3
                                        : 1,
                                },
                            })}
