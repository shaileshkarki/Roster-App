...
function Wages({}) {
  ...
  const handleTotalWagesRosterOne = async (e) => {
    e.preventDefault();
    const response = await axios.get(`${HOST_URL}wages1`, {});
  ...
  const handleTotalRoleWagesRosterOne = async (e) => {
    e.preventDefault();
    const response = await axios.get(`${HOST_URL}wages1/uniqueroles`, {});
    console.log(response.status);
    // if (response.status === 200) {
    //     console.log(response.statusText);
    //     history.replace("/admin");
  ...
  const handlegetGachRosterWeekWagesByEachPersonAndRole = async (e) => {
    ...
    const response = await axios.get(
      `${HOST_URL}wages1/eachrosterswagesbyperson`,
      {}
    );
    console.log(response.status);
    // if (response.status === 200) {
    ...
    <MDBContainer fluid size="12" sm="12" md="12" lg="12" xl="12">
      ...
      <MDBCol sm="12" md="12" lg="12" xl="12">
        ...
        <MDBRow center>
          ...
          <MDBDropdown>
            <Button className="btn btn-primary mr-0">Wages by Job Title</Button>
            <MDBDropdownToggle caret color="primary" />
            <MDBDropdownMenu basic>
              {rosters.map((roster) => (
                <MDBDropdownItem
